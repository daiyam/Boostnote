...
export default class MarkdownPreview extends React.Component {
  ...
  rewriteIframe () {
    ...
    const imgObserver = new MutationObserver(mutationList => {
      for (const mu of mutationList) {
        if (mu.target.className === 'carouselContent-enter-done') {
          this.setImgOnClick(mu.target, rect)

          this.setImgOnDragStart(mu.target)
    ...
    for (const img of imgList) {
      this.setImgOnClick(img, rect)

      this.setImgOnDragStart(img)

      imgObserver.observe(img.parentElement, config)
    }
  }

  setImgOnDragStart(img) {
     img.ondragstart = (e) => {
      const canvas = document.createElement('canvas')
      ...
      // const name = img.src.replace(/^.*\/([^\/]+)\.\w+$/, '$1')
      const mime = data.split(';')[0].split('data:')[1]
      const download = `${mime}:boostnote.png:${data}`

      e.dataTransfer.setData('downloadURL', download)
    }
  }

  setImgOnClick(img, rect) {
    img.onclick = () => {
