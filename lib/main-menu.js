...
const tea = {
	...
	submenu: [
		{
			label: `Generate summary of ${moment().format('YYYY')}'s consumption`,
			accelerator: 'Meta+Shift+T',
			click() {
				mainWindow.webContents.send('editor:generate-current-consumption')
			}
		},
		{
			label: `Generate summary of selected year's consumption`,
			accelerator: 'Meta+Alt+Shift+T',
			click() {
				mainWindow.webContents.send('editor:generate-selected-consumption')
			}
		},
		{
			label: 'Generate reserve summary',
			accelerator: 'Meta+Shift+R',
			click() {
				mainWindow.webContents.send('editor:generate-reserve')
			}
		},
		{
			label: 'Generate purchase summary',
			accelerator: 'Meta+Shift+E',
			click() {
				mainWindow.webContents.send('editor:generate-purchase')
			}
		},
		{
			type: 'separator'
		...
		{
			label: 'Redo best 100ml brewing',
			accelerator: 'Meta+Shift+1',
			click() {
				mainWindow.webContents.send('code:rebrew-best', 100)
			}
		},
		{
			label: 'Redo best 200ml brewing',
			accelerator: 'Meta+Shift+2',
			click() {
				mainWindow.webContents.send('code:rebrew-best', 200)
			}
		},
		{
			label: 'Redo best 400ml brewing',
			accelerator: 'Meta+Shift+4',
			click() {
				mainWindow.webContents.send('code:rebrew-best', 400)
			}
		},
		{
			label: 'Redo last brewing',
			accelerator: 'Meta+Shift+L',
			click() {
				mainWindow.webContents.send('code:rebrew-last')
			}
		},
	]
}
...
module.exports = process.platform === 'darwin'
	? [boost, file, edit, view, window, tea, help]
